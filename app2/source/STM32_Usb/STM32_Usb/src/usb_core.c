...
extern void BSP_TIMER_Delay(__IO uint32_t nDelay_ms);

void USB_OTG_BSP_uDelay(const uint32_t usec)
{
	uint32_t 	count = 0;
	uint32_t	utime = (72 * usec / 7);
...
USB_OTG_STS USB_OTG_EPStartXfer(USB_OTG_CORE_HANDLE *pdev , USB_OTG_EP *ep)
{
	...
	if (ep->is_in == 1)
	{
		...
		USB_OTG_WRITE_REG32(&pdev->regs.INEP_REGS[ep->num]->DIEPTSIZ, deptsiz.d32);   //-1-
