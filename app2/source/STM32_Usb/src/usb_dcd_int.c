...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
	...
	if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
	{
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_outepintr);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_inepint);
		...
		if (gintr_status.b.modemismatch)
		{
			...
			BSP_USB_SAVE_INT(USB_GINTSTS_modemismatch);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_wkupintr);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_usbsuspend);
			
			retval |= DCD_HandleUSBSuspend_ISR(pdev);
		}
		
		if (gintr_status.b.sofintr)    //发送数据中断
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_sofintr);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_rxstsqlvl);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_usbreset);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_enumdone);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_incomplisoin);
		...
		{
			BSP_USB_SAVE_INT(USB_GINTSTS_incomplisoout);
...
static uint32_t DCD_WriteEmptyTxFifo(USB_OTG_CORE_HANDLE *pdev, uint32_t epnum)
{
	...
	if (writeLen == 0 && ep->num == 1)
	{
		uint32_t inEpMsk = 0;
		inEpMsk = 1<<(ep->num);
		USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, inEpMsk, 0);
	}
